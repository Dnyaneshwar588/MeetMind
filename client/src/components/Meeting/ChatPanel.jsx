...
export const ChatPanel = ({ socket, roomId, userId, userName }) => {
  const [messages, setMessages] = useState([]); // Array of { id, text, sender, isBot, isStreaming }
  ...
  useEffect(() => {
    ...
    // Listen when a new answer starts streaming
    socket.on('chat:response:start', ({ userId: senderId, userName: senderName, message, answerId }) => {
      // Add the user's question first (so other room members see it too)
      setMessages((prev) => {
        ...
        if (senderId === userId && prev.some(m => m.text === message && m.sender === senderName)) {
          // Just add the placeholder bot response
          return [
            ...prev,
            {
              id: answerId,
  ...
  const handleSubmit = (e) => {
    ...
    // Add locally to sender's screen immediately for fast response
    // (Other participants will see it via 'chat:response:start')
    setMessages((prev) => [
      ...prev,
      {
  ...
  return (
    <div className="glass-panel rounded-2xl flex flex-col h-[calc(100vh-280px)] border-slate-800/80 overflow-hidden">
      {/* Header */}
      <div className="px-5 py-4 border-b border-slate-800/60 bg-slate-900/40 flex items-center gap-2 justify-between">
        <div className="flex items-center gap-2">
          <Bot size={18} className="text-brand-accent animate-pulse" />
          <h3 className="font-semibold text-slate-200 text-sm">Meeting Co-Pilot</h3>
        </div>
        <div className="flex items-center gap-1 bg-indigo-500/10 border border-indigo-500/20 px-2 py-0.5 rounded-full text-[10px] text-indigo-300">
          <Sparkles size={10} />
          <span>LLaMA 3</span>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="p-4 border-t border-slate-800/60 bg-slate-900/20 flex gap-2">
        <input
          ...
          placeholder="Ask meeting co-pilot..."
